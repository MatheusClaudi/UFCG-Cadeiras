...
public class SortComparatorBSTImpl<T extends Comparable<T>> extends BSTImpl<T> implements SortComparatorBST<T> {
	...
	private void revOrderRec(BSTNode<T> node, List<T> array) {

		if (!node.isEmpty()) {
			revOrderRec((BSTNode<T>) node.getRight(), array);
			array.add(node.getData());
			revOrderRec((BSTNode<T>) node.getLeft(), array);
		}
	}
